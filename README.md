...
```bash
cd <your project>
git clone --depth 1 https://github.com/dm-23/dream-team.git .claude
rm -rf .claude/.git
```

If `.claude/` already exists, clone somewhere else and copy the contents over it instead:
...
Either way you end up with `.claude/agents/`, `.claude/skills/`, `.claude/templates/`, `.claude/hooks/` and `.claude/team-manifest.json`. Commands keep their short names: `/team`, `/generate-knowledge`, `/team-setup`.
...
|-------|-------|-----------|-------------|
| Team core | `.claude/agents`, `.claude/skills`, `.claude/templates`, `.claude/hooks`, `.claude/team-manifest.json` | yes, in this repository | team maintainer |
...
Layer one is a single flat copy of this repository — no wrapper directory, no second copy to keep in sync. Every path the team names is one of the paths above, so the deployment does not depend on where you cloned from or what you called the clone.

Layer two is hidden by the `.gitignore` that ships with the team: it lands at `.claude/.gitignore`, where `knowledge/` is read relative to `.claude/`. Layer three sits at the project root and is hidden through `.git/info/exclude`, which is machine-local; `/team-setup fix` adds that line. Your project's own committed ignore file is never touched.
...
This repository, and equally the project's `.claude/` after deployment:
...
Updating is the install again: take a fresh copy and lay it over `.claude/`.
